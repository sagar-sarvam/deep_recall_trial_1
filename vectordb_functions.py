...
if __name__ == "__main__":
    ...
    metadata = {
        "title": "Chernobyl Disaster Analysis",
        "language": "English",
        "topics": ["Nuclear Accident"],
        "culture": "Soviet",
        "scientificTerms": ["Radioactive decay", "Isotopes"]
    }

    input_doc(doc_id, index_name, text, emb_model, metadata)

    print("\nSearching for text + metadata...")
    # text + metadata search
    get_paras(index_name, ['doc_1'], query_text="workers")

    # metadata-only search
    get_paras(index_name, ['doc_1'], query_text=None)
